...
void seqShrinker( void ) {
	...
	for ( uint64_t seqIdx = 0; seqIdx < sequenceOrg.size(); seqIdx ++ ) {
		uint64_t blocks = sequenceOrg[seqIdx].size() / BLOCKLENGTH;
		// Effective block choice
		uint64_t startS = 0;
		if ( seqIdx % 2 == 0 ) startS = 0;
		else startS = 1;
		// Block selection
		for ( uint64_t cnt = startS; cnt < blocks; cnt = cnt + 2 ) {
			// Terminate or not
			if ( sequenceRdc.size() == seqSizeRdc ) {
				terminate = true;
				break;
			}
			// Set the block
			uint64_t startB = cnt * BLOCKLENGTH;
			string block = sequenceOrg[seqIdx].substr(startB, BLOCKLENGTH);
...
int main() {
	...
	char *filenameOut = "/mnt/ephemeral/hg19Reference256MersFrom1256MBVer2.bin";
