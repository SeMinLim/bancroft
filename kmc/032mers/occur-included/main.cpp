...
bool descendingOrder( pair<uint64_t, uint32_t>& x, 
		      pair<uint64_t, uint32_t>& y ) {
...
void kmc( char *filename ) {
	for ( uint64_t seqIdx = 0; seqIdx < sequences.size(); seqIdx ++ ) {
		...
		while ( start <= sequences[seqIdx].size() - KMERLENGTH ) {
			...
			if ( reference.insert(make_pair(encSubseq, 1)).second == false ) {
				reference.at(encSubseq) += 1;
			} else refSizeOrg ++;
			start += 1;
			// Check the progress
	...
	// Do sorting
	vector<pair<uint64_t, uint32_t>> reference_vector(reference.begin(), reference.end());
	...
	for ( uint64_t i = 0; i < reference_vector.size(); i ++ ) {
		...
		encKmer[1] = (uint64_t)reference_vector[i].second;
...
int main() {
	...
	char *filenameOut = "/mnt/ephemeral/reference/HG19Reference256MersFrom1OccurIncluded.bin";
