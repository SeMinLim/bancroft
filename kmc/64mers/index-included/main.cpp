...
bool descendingOrder( pair<
		      pair<uint64_t, uint64_t>, pair<uint32_t, uint32_t>>& x, 
		      pair<
		      pair<uint64_t, uint64_t>, pair<uint32_t, uint32_t>>& y ) {
...
void kmc( char *filename ) {
	// Do 2-bit Encoding & Put all 64-mers to Map & Count the number of occurrence
	uint32_t index = 0;
	for ( uint64_t seqIdx = 0; seqIdx < sequences.size(); seqIdx ++ ) {
		...
		while ( start <= sequences[seqIdx].size() - KMERLENGTH ) {
			...
			// Put 64-mers, index, and occurrence to the reference
			if ( reference.insert(make_pair(make_pair(encSubseq[0], encSubseq[1]), make_pair(index, 1))).second == false ) {
				reference.at(make_pair(encSubseq[0], encSubseq[1])).second += 1;
			} else refSizeOrg ++;
			start += 1;
			index += 1;
			// Check the progress
	...
	for ( uint64_t i = 0; i < reference_vector.size(); i ++ ) {
		...
		encKmer[1] = reference_vector[i].first.second;
		encKmer[2] = (uint64_t)reference_vector[i].second.first;
...
int main() {
	...
	char *filenameOut = "/mnt/ephemeral/hg19Reference64MersFrom1IndexIncluded.bin";
