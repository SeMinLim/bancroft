...
bool descendingOrder( pair<
		     pair<uint64_t, uint64_t>, uint32_t>& x, 
		     pair<
		     pair<uint64_t, uint64_t>, uint32_t>& y ) {
...
void kmc( char *filename ) {
	for ( uint64_t seqIdx = 0; seqIdx < sequences.size(); seqIdx ++ ) {
		...
		while ( start <= sequences[seqIdx].size() - KMERLENGTH ) {
			...
			if ( reference.insert(make_pair(make_pair(encSubseq[0], encSubseq[1]), 1)).second == false ) {
				reference.at(make_pair(encSubseq[0], encSubseq[1])) += 1;
			} else refSizeOrg ++;
			start += 1;
			// Check the progress
	...
	// Do sorting
	vector<pair<pair<uint64_t, uint64_t>, uint32_t>> reference_vector(reference.begin(), reference.end());
	...
	for ( uint64_t i = 0; i < reference_vector.size(); i ++ ) {
		...
		encKmer[2] = (uint64_t)reference_vector[i].second;
